...
class DasymetricCalculations(object):
    ...
    def updateMessages(self, parameters):
        ...
        if validator:
             return validator(parameters).updateMessages()
    def execute(self, parameters, messages):
        with script_run_as(u'C:\\sync\\DasyToolbox\\DasymetricCalculations.py'):
            # ---------------------------------------------------------------------------
            ...
            def RemoveNulls(tableView,field): 
                ...
                if arcpy.GetCount_management(tableView):
                    arcpy.CalculateField_management(tableView, field, "0")
            ...
            def ClearField(tableName,fieldName):
                ...
                if (len(fields) > 0):
                    ...
                    if field.type == 'String':
                        arcpy.CalculateField_management(tableName,fieldName, "''")
                    else:
                        arcpy.CalculateField_management(tableName,fieldName, "0")
