    ...
    def updateParameters(self, parameters):
        if parameters[0].value:
            ...
            if not parameters[5].altered:
                parameters[5].value = os.path.join(outPath, u"PopRaster" + rastSuffix)
            if not parameters[6].altered:
                parameters[6].value = os.path.join(outPath, u"PopTable" + tblSuffix)
    ...
    def updateParameters(self, parameters):
        if parameters[0].value:
            ...
            if not parameters[2].altered:
                parameters[2].value = os.path.join(outPath, u"DasyRaster" + rastSuffix)
            if not parameters[3].altered:
                parameters[3].value = os.path.join(outPath, u"DasyWorkTable" + tblSuffix)
        validator = getattr(self, 'ToolValidator', None)
        if validator:
             return validator(parameters).updateParameters()
    def updateMessages(self, parameters):
...
class CreateAncillaryPresetTable(object):
    ...
    def updateParameters(self, parameters):
        if parameters[0].value:
            ...
            if not parameters[1].altered:
                parameters[1].value = os.path.join(outPath, u"AncillaryPresets" + tblSuffix)
        validator = getattr(self, 'ToolValidator', None)
        if validator:
             return validator(parameters).updateParameters()
    def updateMessages(self, parameters):
    ...
    def execute(self, parameters, messages):
        try:
            ...
            AddPrintMessage("The table is ready - please populate the PRESETDENS field with the appropriate preset density values, and remove any ancillary classes whose density should be obtained empirically.",1)
...
class LegacyDasymetricCalculations(object):
    """
    # ---------------------------------------------------------------------------
    # DasymetricCalculations.py
    # Part 4 of the Intelligent Areal Weighting Dasymetric Mapping Toolset
    ...
    # <OutFeatureClass> <percent> <sampleMin> 
    #
    # This version of the tool uses the "Frequency Analysis" geoprocessing tool
    # to calculate statistics for individual population units.  This method 
    # might possibly be a better choice for very large datasets because it 
    # writes intermediate tables to disc rather than holding them in memory, but
    # it is definitely much slower than the newer version of this code. We 
    # recommend you try the new method first, and use this one only if you 
    # encounter problems. The output should be identical. 
    # ---------------------------------------------------------------------------
    """
    def __init__(self):
        self.label = u'Legacy Step 4 - Dasymetric Calculations'
        self.canRunInBackground = False
    def getParameterInfo(self):
        # Population_Working_Table
        param_1 = arcpy.Parameter()
    ...
    def execute(self, parameters, messages):   
        ...
        #probably needs to be removed for compatibility with vectors as input.
            ...
            arcpy.CalculateField_management(outWorkTableView, joinedFieldName(outWorkTableView,outWorkTableName,"POP_AREA"), "!" + joinedFieldName(outWorkTableView,inhabAreaTableName,dasyAreaField) + "!", 'PYTHON')
            ...
            # The Frequency tool creates a new table with a row corresponding to each unique combination of population unit and ancillary class, 
            # summing the total area for each - since a population unit might contain multiple separate areas of the same ancillary class.
            arcpy.Frequency_analysis(outWorkTableView, percentAreaTable, popIDField + ";" + ancCatName + ";POP_AREA", dasyAreaField)
            # Add and calculate a field that will contain the percent of the population unit covered by ancillary class
            ...
            for inAncCat, OutAncCat in zip(inAncCatList, outAncCatList):
                whereClause = arcpy.AddFieldDelimiters(percentAreaTable,"percent") + " >= " + str(percent)
                whereClause = whereClause + " AND " + arcpy.AddFieldDelimiters(percentAreaTable,ancCatName) + " = " + str(inAncCat)
            ...
            AddPrintMessage("Calculating statistics for selected classes...",0)
            # Make sure there are representative classes.
            RemoveNulls(popWorkTableView,"REP_CAT")
            whereClause = arcpy.AddFieldDelimiters(popWorkingTable,"REP_CAT") + " <> 0"
            if ancCatFieldProps[0] == "TEXT":
                whereClause = whereClause + " AND " + arcpy.AddFieldDelimiters(popWorkingTable,"REP_CAT") + " <> ''"
            aPopWorkTableView = "aPopWorkTableView"
            arcpy.MakeTableView_management(popWorkingTable, aPopWorkTableView, whereClause)
            ...
            if unSampledList:
                ...
                arcpy.SelectLayerByAttribute_management(outWorkTableView, "NEW_SELECTION", ancCatName + " IN (" + ", ".join(unSampledList) + ")")
                ...
                arcpy.SelectLayerByAttribute_management(outWorkTableView, "CLEAR_SELECTION") # To clear previous selection set - remainder table consolidates all population estimates with all remaining area.
                ...
                whereClause = whereClause + " AND " + arcpy.AddFieldDelimiters(outWorkTableView,joinedFieldName(outWorkTableView,outWorkTableName,"REM_AREA")) + " <> 0" # Still only working with unsampled classes
                '''
                whereClause = joinedFieldName(outWorkTableView,remainderTableName,"POP_DIFF") + " > 0"  # Can't really deal with the overestimates
                whereClause = whereClause + " AND " + joinedFieldName(outWorkTableView,remainderTableName,"REM_AREA") + " <> 0"  # Don't want to divide by zero
                whereClause = whereClause + " AND " + joinedFieldName(outWorkTableView,outWorkTableName,"REM_AREA") + " <> 0" # Still only working with unsampled classes
                arcpy.SelectLayerByAttribute_management(outWorkTableView, "NEW_SELECTION", whereClause)
                ...
                whereClause = whereClause + " AND " + arcpy.AddFieldDelimiters(outWorkTableView,"POP_EST") + " <> 0"
                '''
                whereClause = ancCatName + " IN (" + ", ".join(unSampledList) + ")" + " AND POP_EST <> 0"
                arcpy.SelectLayerByAttribute_management(outWorkTableView, "NEW_SELECTION", whereClause)
                ancDensTable2, ancDensTable2Name = NameCheck("ancDensTable2",tableSuffix)
                arcpy.Frequency_analysis(outWorkTableView, ancDensTable2, ancCatName, "POP_EST;REM_AREA")
                arcpy.AddField_management(ancDensTable2, "CLASSDENS", "DOUBLE")
                whereClause = arcpy.AddFieldDelimiters(ancDensTable2,"POP_EST") + " > 0"
                ancDensTable2View = "ancDensTable2View"
                arcpy.MakeTableView_management(ancDensTable2, ancDensTable2View, whereClause)
                arcpy.CalculateField_management(ancDensTable2View, "CLASSDENS", "!POP_EST! / !REM_AREA!", 'PYTHON')
            ...
            whereClause = joinedFieldName(outWorkTableView,popEstSumTableName,"POP_EST") + " <> 0"
            arcpy.SelectLayerByAttribute_management(outWorkTableView, "NEW_SELECTION", whereClause)
...
class CreateFinalRaster(object):
    ...
    def updateParameters(self, parameters):
        if parameters[0].value:
            ...
            fcSuffix, tblSuffix, rastSuffix = setSuffixes(outPath)
            if not parameters[2].altered:
                parameters[2].value = os.path.join(outPath, u"DensityRaster" + rastSuffix)
        validator = getattr(self, 'ToolValidator', None)
        if validator:
             return validator(parameters).updateParameters()
    def updateMessages(self, parameters):
...
class CombinedSteps123(object):
    ...
    def updateParameters(self, parameters):
        if parameters[0].value:
            ...
            fcSuffix, tblSuffix, rastSuffix = setSuffixes(outPath)
            if not parameters[3].altered:
                parameters[3].value = os.path.join(outPath, u"PopRaster" + rastSuffix)
            if not parameters[4].altered:
                parameters[4].value = os.path.join(outPath, u"PopTable" + tblSuffix)
            if not parameters[7].altered:
                parameters[7].value = os.path.join(outPath, u"AncillaryPresets" + tblSuffix)
            if not parameters[8].altered:
                parameters[8].value = os.path.join(outPath, u"DasyRaster" + rastSuffix)
            if not parameters[9].altered:
                parameters[9].value = os.path.join(outPath, u"DasyWorkTable" + tblSuffix)
        validator = getattr(self, 'ToolValidator', None)
        if validator:
             return validator(parameters).updateParameters()
    def updateMessages(self, parameters):
